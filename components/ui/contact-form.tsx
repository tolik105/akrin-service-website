      ...
      <div className="relative flex flex-col items-center overflow-hidden lg:items-start">
        <div className="flex items-start justify-start">
          <FeatureIconContainer className="flex items-center justify-center overflow-hidden">
            <IconMailFilled className="h-6 w-6 text-zinc-700 dark:text-zinc-300" />
...
export const FeatureIconContainer = ({
  ...
  return (
    <div
      className={cn(
        "relative h-14 w-14 rounded-md bg-gradient-to-b from-zinc-50 to-zinc-200 p-[4px] dark:from-zinc-800 dark:to-zinc-950",
        className,
      )}
    >
      <div
        className={cn(
          "relative z-20 h-full w-full rounded-[5px] bg-zinc-100 dark:bg-zinc-800",
          className,
        )}
      >
        {children}
      </div>
      <div className="absolute inset-x-0 bottom-0 z-30 mx-auto h-4 w-full rounded-full bg-zinc-500 opacity-50 blur-lg"></div>
      <div className="absolute inset-x-0 bottom-0 mx-auto h-px w-[60%] bg-gradient-to-r from-transparent via-zinc-600 to-transparent"></div>
      <div className="absolute inset-x-0 bottom-0 mx-auto h-px w-[60%] bg-gradient-to-r from-transparent via-[rgba(5,45,255,.6)] to-transparent dark:h-[8px] dark:blur-sm"></div>
    </div>
  );
};
...
export const Grid = ({
  ...
  // Use fixed positions instead of Math.random() to avoid hydration mismatches
  const p = pattern ?? [
    [8, 3],
    [9, 2],
    [10, 4],
